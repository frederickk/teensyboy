...
void modeLSDJKeyboardSetup()
{
  ...
  /**
   * The stuff below makes sure the code is in the same state as
   * LSDJ on reset / restart, mode switched, etc.
   */

  // Return LSDJ to the first octave
  for (int rst = 0; rst < 5; rst++)
  {
    sendKeyboardByteToGameboy(keyboardOctDn);
  }

  // Return LSDJ to the first instrument
  for (int rst = 0; rst < 41; rst++)
  {
    sendKeyboardByteToGameboy(keyboardInsDn);
  }
...
void modeLSDJKeyboard()
{
  ...
  { // Loop foreverrrr
    ...
    // Weee hello world bitwise and. ... make the second hex digit zero so we
    // can have a simple case statement - the second digit is usually the midi
    // channel 0 to F (1-16) unless its a 0xF0 message...
    updateStatusLed(); // Update our status blinker
